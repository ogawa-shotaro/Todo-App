...
本番環境では、ローカルで動作確認したアプリケーションを Docker化し、AWS上にデプロイしています。
...
概要
本アプリケーションは、ローカルで動作確認したアプリケーションをDocker化し、AWS上にデプロイします。

※Dockerfileについては、現時点で構成が不明確(知識不足・学習不足による)な為、記載なし。
...
## 8.【セキュリティ設計】
最小権限の原則に基づき、通信経路は以下のようする。
インターネット経由でALB（HTTP / HTTPS）へアクセス→ALBがターゲットグループを通じてECSコンテナへリクエストを転送
→コンテナはアプリケーション処理を実行し、RDSへ接続→RDSはECSからの通信のみを許可
